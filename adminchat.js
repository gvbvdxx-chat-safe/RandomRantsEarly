...
var acl = {
  standard: "_everyone_",
  ok: "_everyone_",
  itsBothering: ["eli_", "gvbvdxx"],
  STOP: ["eli_", "gvbvdxx"],
  adminOnly: ["eli_", "gvbvdxx"],
};

window.annoyingCommands = {
  confetti: acl.ok,
  m: acl.itsBothering,
  uh: acl.itsBothering,
  luig: acl.STOP,
  ...
  scaleY: acl.STOP,
  mute: acl.adminOnly,
  unmute: acl.adminOnly,
};

window.commandInfo = {
  confetti: "(No properties required)",
  m: "<Message>",
  forceOpen: "<URL>",
  ...
  freezeOP: "<Username, @random, @all, or @others>",
  ...
  crash: "<Username, @random, @all, or @others>",
  nyan: "<Username, @random, @all, or @others>",
  play: "<Username, @random, @all, or @others> <Audio URL, must be a direct URL>",
...
window.commands = {
  ...
  spin: function () {
    ...
    var int = setInterval(() => {
      ...
      rotatespeed += 1;
      document.body.style.rotate = rotatedeg + "deg";
      if (rotatedeg > 360) {
  ...
  commands: function (props, msg) {
    if (msg.username == usernameInputElement.value) {
      ...
      adminCommandsServerObject.fakeMessage({
        message:
          "[br][bold]List of possible commands[/bold] [bold]"+message2+"[/bold]:[br];" +
          info.join("[br][br];"),
        color: "green",
        name: "Admin Commands",
      });
    ...
    if (data.username == usernameInputElement.value) {
      adminCommandsServerObject.sendMessage({
        message:
          ...
          " has been kicked because of spam.",
        color: "red",
        name: "Admin Commands",
      });
